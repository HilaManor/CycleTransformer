...
def train(args, dataset, device):
    ...
    for epoch in range(1, args["epochs"] + 1):
        ...
        #for i, (im, txt_tokens, masked_txt_tokens) in enumerate(tqdm(train_loader)):
        for i, (im, _, masked_txt_tokens) in enumerate(tqdm(train_loader)):
            # text to image
        ...
        logline = f"Epoch: {epoch}/{args['epochs']} | " \
        ...
        if epoch % args["val_epochs"] == 0:
            im2txt_running_loss = calc_metrics(im2txt_model, im2txt_criterion, valid_loader, device)
            logline = f"VALIDATION - Epoch: {epoch}/{args['epochs']} | " \
                      f"Im2Txt Loss: {im2txt_running_loss:.4f}"
            print(logline)
            with open(os.path.join(args["output_dir"], 'log.txt'), 'a') as fp:
                fp.write(logline + '\n')      
            
            torch.save({'im2txt': im2txt_model.state_dict(),
...
def calc_metrics(im2txt_model, im2txt_crit, dataloader, device):
    im2txt_model.eval()
    im2txt_running_loss = 0.0
    deTensor = transforms.ToPILImage()
    with torch.no_grad():
        for i, (im, _, masked_txt_tokens) in enumerate(dataloader):
